...
- 性质：一至四节是**只读分析**，成文时未改动 rs-plant 任何文件；
  ...
  **一句话现状：五步里第 1、2 步已完成，第 3 步经核对后撤销，第 5 步的前半插队先做了，
  真正剩下的是第 4 步（拆 5 个「仅删 UI」的面板）与旧 `editor_ui` 外壳。详见第五节**
...
2. 再删 4 个「整体删除」的插件，同步摘掉 `lib.rs` 的 `use` 与 `add_plugins`。
   **已做**（回填 07-27 下午）：四个目标全部清掉。`docs_plugin` 在第一步随那 20 个一起删了；
   `toast_test_plugin`、`controllable_toast_plugin`、`meta_data_plugin` 的源文件与 `mod.rs` 声明
   都在 `e59ddc99` 里删干净了（`git log --diff-filter=D` 逐个核过，`git ls-files` 下这三个路径 0 个文件）。
   现在 `src/plugins/` 下 15 个目录、`mod.rs` 里 15 条声明（原生 14 条，`web_plugin` 带 wasm32 cfg），
   **磁盘上的每个目录都在模块树里，一个都不多**。

   > **本文档第三次栽在同一类判据上，这次是我。** 这一条的初稿写的是「三个目录仍在磁盘上、
   > 是新一批死代码」，依据是 `Get-ChildItem -Directory` 数出 18 个目录。
   > 实际那三个是**空目录**——文件早删了，而 git 不跟踪空目录，所以 `git status` 一片干净，
   > 只有按目录数去数才会多出三个。第一节说判据得是「能不能从模块树走到」而不是「文件在不在」，
   > 第四节说决定运行时行为的是 `add_systems` 写在哪而不是 `fn` 写在哪，
   > 这里再补一条同源的：**数目录不算数，得数目录里有什么**。
   > 三个空壳已顺手清掉，git 无感知、不产生提交。
