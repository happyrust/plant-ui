...
| `rs_surreal/mdb.rs` `MDB_DESI_DBNOS` | `where STYP = $db_type` -> `where type::string(STYP) = type::string($db_type)` | 现在 gen-model 写进库的 `MDB.CURD.STYP` 是字符串 `"1"`，老库是整数 1，而 SurrealQL 里 `"1" = 1` 恒假：库供数对着 8009（ns 1516）解出 **0 个设计库**，根层 0 个 SITE、树面板「当前库下没有 SITE」，三维实例（`inst_relate`）却照常（2026-09-07 实机）。两边转文本再比，两种形状都认；`/ALL` 由此解出 29 个 DESI 库、`SITE` 行落在 7997 / 8000 两个。`plant-ui-data` 的 `mdb_desi_dbnos_compares_styp_as_text_on_both_sides` 钉着这一行文本，重放漏了会红 |
