...
| 画板 | 尺寸 | 已画 | 对应代码 |
|---|---|---|---|
| S1 主工作台（深色） | 1600×1000 | 是 | `dock.rs`、`editor_ui/ui_plugin.rs`、`ui/menu_bar.rs`；2026-09-19 命令栏最左补「导航历史」组（后退 / 前进两枚 C/ToolBtn + 分隔），S3 同步 |
| S1-D 导航历史 · 前进 / 后退形态表 | 1040×720 | 是 | `workbench/chrome.rs` 命令栏那两枚按钮的四种形态（常态 / 双向可用 / 悬停 + hover 文案 / 栈空）与右键最近位置列表；一条历史 = 选中构件 + 相机位姿 + 激活页签（+ 房间聚焦），历史栈在宿主 `main.rs`（`set_selection` / `focus_room` 两处记录），相机位姿由 `plant-ui-view3d` 发布 / 回放；快捷键 Alt+← / Alt+→ 与鼠标侧键（浏览器版由 `web/public/index.html` 取消侧键松开的默认动作，否则 Chrome 拿去翻页面）；2026-09-19 补，桌面 / 浏览器真机各过一遍 |
