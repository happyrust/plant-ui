...
pub(crate) type XlReader<'a, RS: Read + Seek> = XmlReader<BufReader<ZipFile<'a, RS>>>;
...
pub(crate) fn read_string(
    xml: &mut XlReader<'_, impl Read + Seek>,
    QName(closing): QName,
...
fn read_merge_cells(xml: &mut XlReader<'_, impl Read + Seek>) -> Result<Vec<Dimensions>, XlsxError> {
    let mut merge_cells = Vec::new();
