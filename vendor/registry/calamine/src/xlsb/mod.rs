...
impl<RS: Read + Seek> Xlsb<RS> {
    ...
    pub fn worksheet_cells_reader<'a>(
        ...
        let (_, path) = self
            ...
            .iter()
            .find(|&(n, _)| n == name)
            .ok_or_else(|| XlsbError::WorksheetNotFound(name.into()))?;
        XlsbCellsReader::new(
            RecordIter::from_zip(&mut self.zip, path)?,
            &self.formats,
            &self.strings,
            &self.extern_sheets,
            &self.metadata.names,
...
pub(crate) struct RecordIter<'a, RS: Read + Seek> {
    b: [u8; 1],
    r: BufReader<ZipFile<'a, RS>>,
}

impl<'a, RS: Read + Seek> RecordIter<'a, RS> {
    fn from_zip(
