...
type OdsReader<'a, RS: Read + Seek> = XmlReader<BufReader<ZipFile<'a, RS>>>;
...
fn read_table<RS: Read + Seek>(reader: &mut OdsReader<'_, RS>) -> Result<(Range<Data>, Range<String>), OdsError> {
...
fn read_named_expressions<RS: Read + Seek>(reader: &mut OdsReader<'_, RS>) -> Result<Vec<(String, String)>, OdsError> {
    let mut defined_names = Vec::new();
