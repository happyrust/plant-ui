...
impl Window {
    pub(crate) fn new(
        ...
        let inner = Inner { id, window: window.clone(), input, canvas, destroy_fn: Some(destroy_fn) };
...
impl Inner {
    ...
    //todo：实现当 ime 输入法结束时，删除这个 input 元素？
    #[inline]
    pub fn set_ime_cursor_area(&self, position: Position, size: Size) {
        // move dummy input element to this position.
        // log::info!("set_ime_cursor_area : {:?}, {:?}", position, size);
        ...
        if Some(false) == is_mobile() {
            let (x, y): (f32, f32) = position.to_logical::<f32>(self.scale_factor()).into();
            let (w, h): (f32, f32) = size.to_logical::<f32>(self.scale_factor()).into();
            let canvas = self.canvas.borrow();
            let canvas: &HtmlCanvasElement = canvas.raw();
            let x = (x + canvas.offset_left() as f32).min(canvas.width() as f32).max(0.0);
            let y = (y + canvas.offset_top() as f32 - 2.0 * h).min(canvas.height() as f32).max(0.0);
            // log::info!("scale factor: {} ,xy: {x},{y}, canvas: {}, {} - {},{}", self.scale_factor(), canvas.offset_left(), canvas.offset_top(), canvas.width(), canvas.height() );
