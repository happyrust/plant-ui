...
pub struct Input {
    ...
    on_key_up: Option<EventListenerHandle<dyn FnMut(KeyboardEvent)>>,
}
struct Common {
...
impl Input {
    ...
    pub fn on_keyup<F>(&mut self, mut handler: F)
        where
            F: 'static + FnMut(KeyboardEvent),
    {
        ...
        self.on_key_up = Some(
            self.common
                .add_event("keyup", move |event: KeyboardEvent| {
                    if !composing.get() {
                        handler(event)
                    }
                }),
...
impl Common {
            ...
            F: 'static + FnMut(E),
    {
        ...
        let listener = EventListenerHandle::new(self.raw.clone(), event_name, closure);
