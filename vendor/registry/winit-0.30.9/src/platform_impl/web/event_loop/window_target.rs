...
use crate::keyboard::{KeyLocation, ModifiersState};
...
impl ActiveEventLoop {
    ...
    pub fn register_input(&self, input: &Rc<RefCell<backend::Input>>, id: WindowId) {
        ...
        input.on_composition_update(move |text: Option<String>| {
            if let Some(text) = text {
                if !text.is_empty() {
                    ...
                    runner.send_event(super::Event::WindowEvent {
                        window_id: RootWindowId(id),
                        event: WindowEvent::Ime(crate::event::Ime::Preedit(text, Some((len, len)))),
                    });
                }
            }
        });
        ...
        {
            let runner = self.runner.clone();
            input.on_keydown(move |event: KeyboardEvent, composing: bool| {
                // log::info!("keydown event: {:?}", event);
                if !(&event.key() == "Process" || &event.key() == "Unidentified") {
                    let text = backend::key_text(&event);
                    runner.send_event(crate::event::Event::WindowEvent {
                        window_id: RootWindowId(id),
                        event: WindowEvent::KeyboardInput {
                            device_id: RootDeviceId(unsafe { super::device::DeviceId::dummy() }),
                            event: KeyEvent {
                                physical_key: backend::key_code(&event),
                                logical_key: backend::key(&event),
                                text: text.clone(),
        ...
        {
            let runner = self.runner.clone();
            input.on_keyup(move |event: KeyboardEvent| {
                // log::info!("keyup event: {:?}", event);
                if !(&event.key() == "Process" || &event.key() == "Unidentified") {
                    //还差modifiers
                    let text = backend::key_text(&event);
                    runner.send_event(crate::event::Event::WindowEvent {
