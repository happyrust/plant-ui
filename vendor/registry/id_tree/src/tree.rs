...
impl<T> Tree<T> {
    ...
    pub fn insert_with_index(
        ...
        match behavior {
            InsertBehavior::UnderNode(parent_id) => {
                ...
                self.insert_with_parent_by_index(node, parent_id, index)
            }
            InsertBehavior::AsRoot => Ok(self.set_root(node)),
        }
    }
    ...
    fn insert_with_parent_by_index(
        &mut self,
        child: Node<T>,
        parent_id: &NodeId,
        index: usize,
        ...
        let new_child_id = self.insert_new_node(child);
        self.set_as_parent_and_child_with_index(parent_id, &new_child_id, index);
        Ok(new_child_id)
    }
    ...
    fn set_as_parent_and_child_with_index(&mut self, parent_id: &NodeId, child_id: &NodeId, index: usize) {
        self.get_mut_unsafe(parent_id).insert_child(child_id.clone(), index);
