...
use crate::parsed_data::{CateProfileParam, CateGeomsInfo};
...
use crate::tool::math_tool::{dquat_to_pdms_ori_xyz_str, quat_to_pdms_ori_str, to_pdms_ori_str, to_pdms_vec_str};
use anyhow::anyhow;
...
pub async fn create_profile_geos(refno: RefnoEnum,
    ...
    let mut spine_paths = if type_name == "GENSEC" || type_name == "WALL" {
        ...
        for &spine_refno in children_refnos.iter() {
            ...
            drne = spine_att.get_dvec3("DRNE").map(|x| inv_mat.transform_vector3(x.normalize()));
            if drne.is_some() && drne.unwrap().is_nan(){
                drne = None;
            }
            // dbg!((drns, drne));
            let ch_atts = crate::get_children_named_attmaps(spine_refno).await.unwrap_or_default();
            let len = ch_atts.len();
            if len < 1 { continue; }
